     ...
     public function boot(){
        view()->composer(['property-management::propertyinvoice.view'], function ($view)
        {
            if(\Auth::check())
            {
                ...
                if(module_is_active('Audit', \Auth::user()->created_by) && (isset($company_settings['audit_is_on']) ? $company_settings['audit_is_on'] : 'off') == 'on' && !empty($company_settings['audit_key']) && !empty($company_settings['audit_secret']))
                {
                    $view->getFactory()->startPush('company_property_payment', view('audit::payment.property_payment'));
                }
            }
        ...
        view()->composer(['property-management::frontend.checkout'], function ($view)
        {
                ...
                {
                    $view->getFactory()->startPush('property_payment', view('audit::payment.property_booking_payment',compact('slug')));
                }
        });
    }
